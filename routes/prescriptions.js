...
const { notifyPrescriptionReviewed } = require('../services/notifications');
...
router.patch('/:id/verify', authenticateToken, requireRole('admin', 'pharmacist'), async (req, res, next) => {
  try {
    ...
    if (!rxSnap.exists) return res.status(404).json({ error: 'Prescription not found.' });
    ...
    notifyPrescriptionReviewed({ ...rxSnap.data(), status, rejectionReason });
